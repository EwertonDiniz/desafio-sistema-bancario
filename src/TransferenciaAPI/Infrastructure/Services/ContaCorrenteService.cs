...
public class ContaCorrenteService : IContaCorrenteService
{
    ...
    public async Task<string> ObterIdPorNumeroAsync(string numeroConta)
    {
        var response = await _httpClient.GetAsync($"/api/Conta/id/{numeroConta}");
        if (!response.IsSuccessStatusCode)
            throw new Exception("Erro ao obter id da conta corrente.");

        var id = await response.Content.ReadAsStringAsync();
        return id;
    }

    public async Task<string> DebitarAsync(string contaOrigemNumero, decimal valor)
    {
    ...
    public async Task<string> CreditarAsync(string contaDestinoNumero, decimal valor)
    {
