...
namespace ContaCorrenteAPI.Controllers
{
    ...
    public class ContaController : ControllerBase
    {
        ...
        public async Task<IActionResult> Debitar([FromBody] MovimentacaoDto dto)
        {
            ...
            var saldoAtual = movimentos.Sum(m => m.Tipo == "C" ? m.Valor : -m.Valor);

            if (saldoAtual < dto.Valor)
                return BadRequest(new { tipo = "INVALID_VALUE", mensagem = "Saldo insuficiente." });

            await _movimentoRepo.RegistrarAsync(new Domain.Entities.Movimento
            {
                ContaId = conta.Id,
                Valor = dto.Valor,
                Tipo = "D",
        ...
        [HttpGet("id/{numeroConta}")]
        public async Task<IActionResult> ObterIdPorNumero(string numeroConta)
        {
            var conta = await _contaRepo.ObterPorNumeroAsync(numeroConta);
            if (conta == null)
                return NotFound(new { tipo = "INVALID_ACCOUNT", mensagem = "Conta não encontrada." });
