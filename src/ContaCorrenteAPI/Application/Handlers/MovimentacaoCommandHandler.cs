...
namespace ContaCorrenteAPI.Application.Handlers
{
    public class MovimentacaoCommandHandler : IRequestHandler<MovimentacaoCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(MovimentacaoCommand request, CancellationToken cancellationToken)
        {
            ...
            var movimentosExistentes = await _movimentoRepo.ObterPorContaIdAsync(conta.Id);
            var saldoAtual = movimentosExistentes.Sum(m => m.Tipo == "C" ? m.Valor : -m.Valor);

            if (tipo == "D" && saldoAtual < request.Valor)
                throw new ValidationException("INVALID_VALUE: Saldo insuficiente.");
